...
class RoomWrapper:
    ...
    def get_room_type_by_id(self, id_room_type):
        cursor = self.connection.cursor(pymysql.cursors.DictCursor)
        sql = "SELECT * FROM `room_type` WHERE id = {}".format((id_room_type))
        cursor.execute(sql)
        return cursor.fetchone()
    
    def get_room_type_by_name(self, room_type_name):
        cursor = self.connection.cursor(pymysql.cursors.DictCursor)
        sql = "SELECT * FROM `room_type` WHERE name = '{}'".format((room_type_name))
        cursor.execute(sql)
        return cursor.fetchall()
    ...
    def get_room_by_id(self, id):
        cursor = self.connection.cursor(pymysql.cursors.DictCursor)
        sql = "SELECT * FROM room WHERE id = {}".format(id)
        cursor.execute(sql)
        return cursor.fetchone()
    ...
    def update_room_type(self, type_id, employee_id):
        updatedstat = 0
        ...
        cursor = self.connection.cursor(pymysql.cursors.DictCursor)
        sql = 'UPDATE room_type SET status = {}, last_update = NOW(), last_update_by = {} WHERE id = {}'.format(updatedstat, employee_id, type_id)
        cursor.execute(sql)
    ...
    def update_room(self, type_id, idlogin):
        ...
        cursor = self.connection.cursor(pymysql.cursors.DictCursor)
        sql = 'UPDATE room SET status = {}, last_update = NOW(), last_update_by = {} WHERE id = {}'.format(updatedstat, idlogin, type_id)
        cursor.execute(sql)
